  ...
  function initIndex() {
    ...
    request.addEventListener("load", function(event) {
      ...
      index = lunr(function() {
        ...
        this.field("tags");
        // NEW: Add custom fields for OSLA blog
        this.field("author");
        this.field("college");
  ...
  function search(term, doNotAddState) {
    try {
      ...
      for (let result of results) {
          ...
          // Build the post entry with author and college info
          let postEntry = element.querySelector(".post-entry");
          let metaInfo = [];
          if (doc.author) {
            metaInfo.push(doc.author);
          }
          if (doc.college) {
            metaInfo.push(doc.college);
          }
          if (doc.society_role) {
            metaInfo.push(doc.society_role);
          }

          let metaHtml = '';
          if (metaInfo.length > 0) {
            metaHtml = '<p class="text-muted small mb-2"><em>' + metaInfo.join(' • ') + '</em></p>';
          }

          postEntry.innerHTML = metaHtml + '<p>' + truncateToEndOfSentence(doc.content, 70) + '</p>';
